...
def notify_ban(bot, user_id, ban_hours):
    text = f'Congratulations! You were banned for flooding for the number of hours: {ban_hours}.'
...
def _callback(bot, update):
    ...
    if query.data == 'partners_excel':
        if user_request_excel_too_often(user_id, _partners_excel_query_time):
            text = 'You cannot request excel partners more than once a minute.'
            bot.send_message(chat_id=user_id, text=text)
            return
        ...
        if user_request_excel_too_often(user_id, _transactions_excel_query_time):
            text = 'You cannot request excel transactions more than once per minute.'
            bot.send_message(chat_id=user_id, text=text)
            return
...
class MainMenu:
    ...
    def partners(bot, user):
        keyboard = [
            [
                InlineKeyboardButton("Download excel partner table", callback_data='partners_excel'),
            ],
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
